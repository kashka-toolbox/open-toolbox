            ...
            <CommandGroup key={index} heading={category.title}>
              {category.items.map((subItem, subIndex) => (
                <CommandItem
                  ...
                  key={subIndex}
                  className="hover:cursor-pointer flex flex-row items-baseline overflow-hidden text-nowrap"
                >
                  ...
                  <span className="text-muted-foreground pl-2 text-xs text-ellipsis overflow-hidden min-w-0">{subItem.description}</span>
